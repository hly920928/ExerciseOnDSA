...
int threeTupleMinDistance(int a[], int  b[], int  c[], int m, int n, int l) {
	...
	while(true) {
		...
		while (a[p1] < now&& p1 != m - 1)p1 = min(p1+1, m - 1);
		while (b[p2] < now && p2 !=n - 1)p2= min(p2+1, n - 1);
		while (c[p3] < now&& p3 !=l - 1)p3 = min(p3+1, l - 1);
		ans = min(ans, max(a[p1], max(b[p2], c[p3])) - now);
		if (!now_up)break;
		if (now_up ==1) { p1 = min(p1 + 1, m - 1); }
		else if (now_up == 2){ p2 = min(p2 + 1, n - 1); }
		else if (now_up ==3) { p3 = min(p3 + 1, l - 1); }
	}
	return ans;  
}